...
const buttonClick=(e)=>{
  ...
  // if(numberCounter===3){
  //   console.log('YES 3');
  // let res=result.textContent.toString().replace(/\B(?=(\d{2})+(?!\d))/g, ",");
  ...
  console.log('Length is',result.textContent.slice(-1));
...
function getFormatedNumber(number){
  ...
  console.log(number === NaN ? 0 : number);

}

getFormatedNumber(result.textContent)

// result.innerHTML=new Intl.NumberFormat('en-US').format(result.textContent);
...
// console.log(numberCounter);
    ...
    //   var commas = result.innerHTML.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    //   // result.innerHTML=commas; 
    //   console.log(commas);
...
const first=document.querySelector('.first-oper');
